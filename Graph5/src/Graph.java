...
/*	public static List<Integer> updateServerLocation(List<Integer> serverLocation,Map<Integer,Integer> serverProvide,List<Map.Entry<Integer, Integer>> infoIds,int serveridx){
			int provide = Integer.MAX_VALUE;
			int idx = 0;
			for(Map.Entry<Integer, Integer> entry:serverProvide.entrySet()){
				if(entry.getValue()<provide){
					provide = entry.getValue();
					idx = entry.getKey();
				}
				
			}
					
			serverLocation.remove((Integer)idx);
			
			for(int i=serveridx;i<infoIds.size();i++){
				int key = infoIds.get(i).getKey();
				if((key!=idx) && !serverLocation.contains(key)){
	...
	//失败之后更新服务器位置的方法，保持服务器的个数不变
	public static List<Integer> updateServerLocation(List<Integer> serverLocation,Map<Integer,Integer> serverProvide,List<Map.Entry<Integer, Integer>> infoIds,List<Integer> unComplement ){
		int number  = serverLocation.size();
		serverLocation.clear();
		
		
		//先将上一步未满足要求的节点加入作为服务器
		int j=0;
		for(;j<unComplement.size() && j<number;j++){
			serverLocation.add(unComplement.get(j));
		}
		...
		//再从利用率高的服务器节点中选取一部分
		for(int i=0;i<number-unComplement.size()-3;i++){
		...
		for(int i=0;i<infoIds.size();i++){
			int key = infoIds.get(i).getKey();
			if(!serverLocation.contains(key) && !serverProvide.containsKey(key)){
				serverLocation.add(key);
	...
	public static List<Integer> updateServerLocation(List<Integer> serverLocation,Map<Integer,Integer> serverProvide,List<Map.Entry<Integer, Integer>> infoIds,List<Integer> unComplement,int serverNum){
		int number  = serverNum;
		serverLocation.clear();
		...
		for(int i=0;i<sortedServerProvide.size();i++){
			...
			if(!serverLocation.contains(key)){
				serverLocation.add(key);
				if(serverLocation.size()==number-unComplement.size()-10){
					break;
				}
	...
	public static void minCost() throws IOException{
		...
		BufferedWriter bw = new BufferedWriter(new FileWriter(new File("C:/Users/Administrator/Desktop/check3.txt")));
	    ...
	    //初始化服务器的位置
    	List<Integer> serverLocation = Graph.initializeServerLocation(infoIds,initServerNum);//初始化服务器的位置
    	...
    	for(int rd=initServerNum;rd<=initServerNum+10;rd++){
				...
				while(times<=recursiveNum){					
					...
					for(int m=serverLocation.size()-1;m>=0;m--){
						...
						if(!right){
							System.out.println("边删除出错,程序结束运行");
							return;
						}
					}			
					...
					//重新选择服务器
					System.out.println("服务器个数为"+rd+ " ===========第 "+times + "======更新服务器位置===");
					serverLocation = updateServerLocation(serverLocation,serverProvide,infoIds,unComplement,rd);
...
/*	 public static List<Integer> initializeServerLocation(List<Map.Entry<Integer, Integer>> infoIds,int serverNum){	
	 ...
	 */
	 public static void main(String[] args) throws IOException {  
		 ...
		 String[] data = Graph.readData("C:/Users/Administrator/Desktop/Primary/case2.txt");
