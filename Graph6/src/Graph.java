...
public class Graph {
	...
	public static void minCost() throws IOException{
		...
		BufferedWriter bw = new BufferedWriter(new FileWriter(new File("C:/Users/Administrator/Desktop/test2check.txt")));
	   ...
	   // int initServerNum = Graph.consume.size()/2;
	    ...
	    int[] arr = new int[]{9,12,23,4,18,20,17,14,6,26,8,25};
	    List<Integer> serverLocation = new ArrayList<Integer>();
	    for(int i=0;i<arr.length;i++){
	    	serverLocation.add(arr[i]);
	    }
		...
		while(times<=recursiveNum){
			...
			if(Graph.isSuccess){
				int a = Graph.cost+serverLocation.size()*Graph.servicePrices;
				if(Graph.minCost>=a){
					Graph.minCost = a;
					
					String str5 = "该次寻找成功，得到的最小费用为："+Graph.minCost;
					bw.write(str5);
					bw.newLine();
					bw.flush();
				}
