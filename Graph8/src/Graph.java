...
public class Graph {
	...
	public static boolean bellManford(int s,int t,List<Integer> lst,Map<Integer,Integer> serverProvide,Map<Integer,Integer> consumeGet){
		//初始化vis和dis
	...
	public static List<Integer> moreNearbyNode(int serverId){
		...
		return lst;//idx==-1时代表没找到
			
	}

	public static List<Integer> updateServerLocation(List<Integer> serverLocation,List<Integer> sortedKey,List<Integer> discard,List<Integer> unComplement){
	...
	//该次更新，不变服务器个数，只变位置
	public static List<Integer> updateServerLocationDemo1(List<Integer> serverLocation,List<Integer> sortedKey,Map<Integer,Integer> discard,List<Integer> unComplement){
		int number  = serverLocation.size();
		serverLocation.clear();		
		...
		for(;i<sortedKey.size()-discardNum ;i++){
			...
			if(!serverLocation.contains(key)){
				serverLocation.add(key);
				if(serverLocation.size()==number){//如果服务器节点已到个数，跳出
					break;
				}
			}
		}
		...
		//从剔除的节点中选择它临近的加入进去
		List<Integer> inNear = new ArrayList<Integer>();
		for(int m=0;m<temp.size() && serverLocation.size()<number ;m++){//只有服务器个数小于number时才加入
			List<Integer> st = Graph.moreNearbyNode(temp.get(m));
			for(int d=0;d<st.size();d++){
				int in = st.get(d);
				if(discard.containsKey(in) && discard.get(in)>4){//3
					continue;//出现过3次都被抛弃了，那就不要他了
				}
				if(!serverLocation.contains(in)){
					serverLocation.add(in);	
					inNear.add(temp.get(m));
				}
			}
		}
		
		//保险的情况，serverLocation.size()<number，则从已经剔除的里面加入一些进来,当然，该剔除的节点的邻居如果在上面已经加入了，则该剔除的节点也不能加入
		if(serverLocation.size()<number){
			for(int m=0;m<temp.size() && serverLocation.size()<number ;m++){
				int in = temp.get(m);
				if(!serverLocation.contains(in) && !inNear.contains(in)){
					serverLocation.add(in);		
				}
			}
		}
	...
	public static void anasys(Map<Integer,Integer> consumeGet,Map<Integer,Integer> consumeMap,List<Integer> unComplement) throws IOException{
...
/*				String str4 = "=======未满足要求的消费节点分析：";
	...
	public static int statistic(int size) {
		if(Graph.isSuccess){
			int serverNumber = size;
	...
	public static void minCost() throws IOException{
		...
		while(serverLocation.size()>=standard-5 && gg<150){					
			...
			while(bellManford(s,t,lst,serverProvide,consumeGet) && !Graph.isSuccess){
			...
			int nomCost = statistic(serverLocation.size());	
			...
			//根据分析信息，重新选择服务器
			System.out.println("服务器个数为"+serverLocation.size()+"======更新服务器位置===");		
		...
		int times = 0,recusive = 100;
		

		//int idx = 0;
		while(times<allTimes-gg){
			
			System.out.println("循环减少服务器的个数看看");
			serverLocation.clear();
			//先减少一个看看
	...
	public static int contemporyMinCost = Integer.MAX_VALUE;
	public static void searchInfo(int nomCost){
	...
	public static List<Integer> mapSort(Map<Integer,Integer> serverProvide){
	 ...
	 public static void main(String[] args) throws IOException {  
		 ...
		 String[] data = Graph.readData("C:/Users/Administrator/Desktop/Advanced/case0.txt");
