...
public class Graph {
	...
	public static void upateServerLocationDemo1(List<Integer> serverLocation,int serverIdx){
		
		//将为满足要求的节点先加入服务器
		for(int j=serverIdx;j>=0;j--){
			int key = serverLocation.get(j);
			//获取它附近的且性价比较高的节点
			int near = Graph.nearbyNode(key);
			if(near!=-1 && !serverLocation.contains(near)){//由第二步保证了某个服务器节点的附近应该不会再有服务器节点
				serverLocation.remove((Integer)key);
				serverLocation.add(near);
				break;
			}
...
/*		long now1 = System.currentTimeMillis();
		...
		//第三层循环，服务器个数不变的情况下，替换服务器的位置，看看能不能再减少费用	
		if(Graph.vertexNum<500){
			serverLocation.clear();
...
/*		long now = System.currentTimeMillis();
		...
		System.out.println("第二层循环最后一次选用的服务器个数为："+serverLocation.size());
		System.out.println("第二层循环最佳情况下选用的服务器个数为："+Graph.serverProvideCopy.size());
		
		long now3 = System.currentTimeMillis();
		System.out.println("共耗时："+(now3-start));
