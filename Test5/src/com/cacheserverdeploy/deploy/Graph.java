...
public class Graph {
	...
	//这是进行替换操作，serverLocation是按照供应量从大到小排序的，每次替换大的值
/*	public static void upateServerLocationDemo1(List<Integer> serverLocation,int time,int overall,List<Integer> nearByServer){				
		serverLocation.remove(serverIdx);
		serverLocation.add(nearByServer.get(overall));				
...
/*			long now2 = System.currentTimeMillis();
			...
			*/
			...
			if(nearByServer==null){
				...
				nearByServer = Graph.nearbyNode3(serverId);
			}
			...
			if(overall<nearByServer.size() && overall<6){			
				...
				serverLocation.clear();
				serverLocation.addAll(serverProvideCopy);//保存上一次的最好结果，为下一步的服务器替换做准备工作
		...
		System.out.println("第四层循环后的最小费用为："+Graph.minCost);
	...
	/*	//第五层循环，采用启发式的看看结果
		//第一步在上述最优的解旁边放置多个相邻的服务器节点
		if(Graph.vertexNum<500){
			serverLocation.clear();
			serverLocation.addAll(Graph.serverProvideCopy);
			//如果成功更新服务器的位置
			Graph.upateServerLocationDemo3(serverLocation);							
		}
		
		List<Integer> serverLocationCopy = new ArrayList<Integer>(serverLocation);
	
		serverLocation.clear();
		serverLocation.addAll(serverProvideCopy);	
		
		List<Integer> setOut = new ArrayList<Integer>();
		List<Integer> setIn = new ArrayList<Integer>();
		Map<Integer,Double> poolMap = new HashMap<Integer,Double>();//维护一个概率池
		//初始化概率池
		Graph.poolInitial(poolMap, serverLocation, serverLocationCopy);
		Graph.proPoolInitial(poolMap,serverLocation,setOut,setIn);
		
		contemporyMin = Integer.MAX_VALUE;
		int times1 = 0;		
		int g3 = 0;
		int totalLoop3 = 1000;	
		while(g3<totalLoop3){
			...
			if(Graph.isSuccess){
				...
				if(total<contemporyMin){
					...
					Graph.proPoolSuccess(poolMap, serverLocation, setOut, setIn);
	
					
				}else{//如果费用比之前大，接下来更新服务器的位置，并维护概率池
					Graph.proPoolFail(poolMap,serverLocation,setOut,setIn);
			...
			//times1++;//运行到减少服务器的个数的时候，才开始计算减少times
		...
		System.out.println("第五次循环一共循环的次数：" + g3);
		System.out.println("第五次循环找到的局部最小费用：" + contemporyMin);
		System.out.println("第五次循环的最优服务器个数：" +serverProvideCopy.size());
	...
	public static void poolInitial(Map<Integer,Double> poolMap,List<Integer> serverLocation,List<Integer> serverLocationCopy){
		
		int size = serverLocation.size();
		for(int i=0;i<size;i++){
			poolMap.put(serverLocation.get(i), 2.0/serverLocationCopy.size());
		}
		
		for(int i=size;i<serverLocationCopy.size();i++){
			poolMap.put(serverLocationCopy.get(i), 1.0/serverLocationCopy.size());
		}
		
		List<Map.Entry<Integer, Double>> prosort = Graph.proSort(poolMap);
		
		poolMap.clear();
		
		//对调整后的概率进行归一化
		double sum=0;
		for(int i=0;i<prosort.size();i++){
			double value = prosort.get(i).getValue();
			sum += value;
		}
		
		for(int i=0;i<prosort.size();i++){
			int key = prosort.get(i).getKey();
			double value = prosort.get(i).getValue();
			double normalValue = value/sum;
			poolMap.put(key, normalValue);
		}
		
		
	}
	
	public static List<Map.Entry<Integer, Double>> proSort(Map<Integer,Double> poolMap){
		List<Map.Entry<Integer, Double>> prosort = new ArrayList<Map.Entry<Integer,Double>>(poolMap.entrySet());
		
		//对prosort按照从大到小排序
		Collections.sort(prosort,new Comparator<Map.Entry<Integer, Double>>(){
	...
	public static void proPoolSuccess(Map<Integer,Double> poolMap,List<Integer> serverLocation,List<Integer> setOut,List<Integer> setIn){
		
		//如果成功，奖励替补的服务器
		int locationSize = serverLocation.size();
		for(int i=0;i<setIn.size();i++){
			double pro = poolMap.get(setIn.get(i));
			poolMap.put(setIn.get(i), (pro+1.0/poolMap.size()));
		}
		//惩罚丢弃的服务器
		for(int i=0;i<setOut.size();i++){
			poolMap.put(setOut.get(i), 0.0);//将其概率惩罚为0，这个后面可以调整
		}
		...
		//先清理
		serverLocation.clear();	
		setOut.clear();//setOut存放最新一次被抛弃的服务器	
		setIn.clear();
		int i=0;
		for(;i<locationSize-3;i++){
			int key = prosort.get(i).getKey();
			double value = prosort.get(i).getValue();
			/*double normalValue = 1.0*value/sum;
			poolMap.put(key, normalValue);*/
			serverLocation.add(key);
		}
		
		for(;i<locationSize;i++){
			int key = prosort.get(i).getKey();
			setOut.add(key);
...
/*			serverLocation.add(key);*/
		...
		for(int j=locationSize;j<=locationSize+2;j++){
			
			int key = prosort.get(j).getKey();
	...
	public static void proPoolFail(Map<Integer,Double> poolMap,List<Integer> serverLocation,List<Integer> setOut,List<Integer> setIn){
		//如果失败，奖励原来抛弃的服务器
		int locationSize = serverLocation.size();
		for(int i=0;i<setOut.size();i++){
			double pro = poolMap.get(setOut.get(i));
			poolMap.put(setOut.get(i), pro+0.2/poolMap.size());//
		}
		//惩罚加入的服务器
		for(int i=0;i<setIn.size();i++){
			poolMap.put(setIn.get(i), 0.0);//将其概率惩罚为0，这个后面可以调整
		}
...
/*			serverLocation.add(key);*/
	...
	public static void proPoolInitial(Map<Integer,Double> poolMap,List<Integer> serverLocation,List<Integer> setOut,List<Integer> setIn){
...
/*		//踢掉一个性价比最低的
		int sum=0;
		for(int i=0;i<prosort.size()-3;i++){
			double value = prosort.get(i).getValue();
			sum += value;
		}*/
		//将概率最小的几个用其临近的代替
