...
public class Graph {
	...
	//写一个重载方法，为了后面剔除服务器节点使用
	public static boolean bellManford(int s,int t,List<Integer> lst,List<Integer> band){
		//初始化vis和dis
		Arrays.fill(dis, Integer.MAX_VALUE);
		Arrays.fill(vis, 0);//上述两个数组的初始化
	...
	public static List<Integer> updateServerLocation(List<Integer> serverLocation,List<Integer> sortedKey,Map<Integer,Integer> discard,List<Integer> unComplement){
		...
		for(int m=0;m<temp.size()-2;m++){//之前取得是2
	...
	//根据起始点返回正向边
	public static Edge getEdge(int from,int to){
		List<Integer> idx = Graph.graph.get(from);
		for(int h=0;h<idx.size();h++){
			Edge ed = edges.get(idx.get(h));
			if(ed.cap>0 && ed.v==to){
				return ed;
			}
		}
	...
	public static Map<List<Integer>,Integer> route(List<List<Integer>> result){
		Map<List<Integer>,Integer> finalMap = new HashMap<List<Integer>,Integer>();
		for(int i=0;i<result.size();i++){
			List<Integer> r = result.get(i);
			int minFlow = Integer.MAX_VALUE;
			for(int m=r.size()-1;m>=1;m--){
				Edge edge = Graph.getEdge(r.get(m),r.get(m-1));
				if(edge.flow == 0){
					minFlow = 0;
					break;
			...
			if(minFlow>0 && minFlow< Integer.MAX_VALUE ){
				finalMap.put(r, minFlow);
				for(int m=r.size()-1;m>=1;m--){
					Edge edge = Graph.getEdge(r.get(m),r.get(m-1));
		...
		while(serverLocation.size()>=standard  && g1<totalLoop){					
			...
			if(Graph.isSuccess){
				...
				if(total<Graph.minCost){
					...
					Map<List<Integer>,Integer> finalMap = Graph.route(result);
					finalResult.clear();
					finalResult.add(finalMap.size()+"");
					
					for(Map.Entry<List<Integer>, Integer> entry:finalMap.entrySet()){
	...
	public static String[] readData(String path) throws FileNotFoundException{
	  ...
	  */
	  ...
	  public static void main(String[] args) throws IOException {
		  String[] data = Graph.readData("C:/Users/Administrator/Desktop/Primary/case3.txt");
		  Graph.initializeGraph(data);
		  List<String> finalResult = Graph.minCost();	
		  String[] res= new String[finalResult.size()+1];
