...
public class Graph {
	...
	public static void updateServerLocation(List<Integer> serverLocation,List<Integer> sortedKey,Map<Integer,Integer> discard,List<Integer> unComplement){
		...
		for(int m=0;m<temp.size()-2;m++){//折以前是2
	...
	public static List<String> minCost(){
		...
		if(Graph.vertexNum<500){
			...
			long now4 = System.currentTimeMillis();
			System.out.println("第三层循环已经用的时间"+(now4-start));
			System.out.println("第三层循环的次数"+g3);
			System.out.println("第三层循环的最小费用："+Graph.minCost);
		}
		...
		///////////////////////
		////////////////////
		List<List<Integer>> manyGoodServer = new ArrayList<List<Integer>>();
		//第三层循环，找精确服务器的数量	
		...
		long now = System.currentTimeMillis();
		System.out.println("第二层循环已经用的时间"+(now-start));
		System.out.println("第二层循环的次数"+g2);
		System.out.println("第二层循环的最优服务器个数："+serverProvideCopy.size());
		...
		if(Graph.vertexNum<500){
			serverLocation.clear();
			serverLocation.addAll(serverProvideCopy);
			loop4(serverLocation,start,s,t);
			
/*			int rec = manyGoodServer.size();		
			for(int i=rec-1;i>=rec-10 && i>=0;i--){
				List<Integer> to = manyGoodServer.get(i);
				serverLocation.clear();
				serverLocation.addAll(to);
				loop4(serverLocation,start,s,t);
...
/*		serverLocation.clear();
		serverLocation.addAll(serverProvideCopy);*/
		List<Integer> nearByServer = null;
		List<Integer> contemparyBestCopy = new ArrayList<Integer>();
		int overall = 0;
		
		int contemporyMin = Integer.MAX_VALUE;
		int times2 = serverLocation.size()-1;		
		int g4 = 0;
		int totalLoop4 = 200;	
		while(times2>=0 && g4<totalLoop4){
...
/*			long now2 = System.currentTimeMillis();
			...
			if(Graph.isSuccess){
				...
				if(total<contemporyMin){					
					...
					//保存暂时最好的
					contemparyBestCopy.clear();
					contemparyBestCopy.addAll(serverLocation);
					//保存此时的服务器位置
			...
			if(overall<nearByServer.size() && overall<6){			
				...
				serverLocation.clear();
				serverLocation.addAll(contemparyBestCopy);//保存上一次的最好结果，为下一步的服务器替换做准备工作
		...
		System.out.println("第四层循环的次数"+g4);
		System.out.println("第四层循环的最小费用："+contemporyMin);
		System.out.println("第四层循环的最优服务器个数："+contemparyBestCopy.size());
	}
	...
	public static void proPoolSuccess(Map<Integer,Double> poolMap,List<Integer> serverLocation,List<Integer> setOut){
		...
		//踢掉最后一个概率最低的
		double sum=0;
		for(int i=0;i<prosort.size()-1;i++){
		...
		int key = prosort.get(prosort.size()-1).getKey();
		setOut.add(key);//setOut存放最新一次被抛弃的服务器
		
		
	}
	
	public static void proPoolFail(Map<Integer,Double> poolMap,List<Integer> serverLocation,List<Integer> setOut){
		int find = setOut.get(0);
		setOut.clear();//setOut存放最新一次被抛弃的服务器
		
		//获取poolMap中的最大值
		double maxRatio = -1;
		for(Map.Entry<Integer, Double> entry:poolMap.entrySet()){
			if(maxRatio<entry.getValue()){
				maxRatio = entry.getValue();
			}
		}
		double increase = maxRatio;
		
		if(poolMap.containsKey(find)){
			System.out.println("出错了==============================================");
			return ;
		}
		poolMap.put(find,increase);
		List<Map.Entry<Integer, Double>> prosort = Graph.proSort(poolMap);
	...
	public static void proPoolInitial(Map<Integer,Double> poolMap,List<Map.Entry<Integer, Double>> sortQuality,List<Integer> serverLocation,List<Integer> setOut){
		
		setOut.clear();

		//踢掉一个性价比最低的
		int sum=0;
		for(int i=0;i<sortQuality.size()-1;i++){
			double value = sortQuality.get(i).getValue();
			sum += value;
		}
		
		for(int i=0;i<sortQuality.size()-1;i++){
			int key = sortQuality.get(i).getKey();
			double value = sortQuality.get(i).getValue();
			double normalValue = 1.0*value/sum;
			poolMap.put(key, normalValue);
			serverLocation.add(key);
		}
		int key = sortQuality.get(sortQuality.size()-1).getKey();
		setOut.add(key);
