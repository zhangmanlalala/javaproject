...
public class Graph {
	...
	public static Map<Integer,Double> map3 = new HashMap<Integer,Double>();
	//public static Map<Integer,Double> map4 = new HashMap<Integer,Double>();
	...
	public static int nearbyNode(int serverId){
		...
		for(int i=0;i<num;i++){
			...
			double linkedNodeWeight = Graph.map3.get(linkedNode);//获取serverId附近性价比最高的节点
	...
	public static double[] nearbyNode0(int serverId){
		...
		if(idx!=-1){
			return new double[]{idx,max};//idx==-1时代表没找到
	...
	public static List<Integer> nearbyNode3(int serverId){
		...
		//从大到小排序
		List<Map.Entry<Integer, Double>> stemp = new ArrayList<Map.Entry<Integer,Double>>(mapTemp.entrySet());
		Collections.sort(stemp,new Comparator<Map.Entry<Integer, Double>>(){
	...
	public static List<Integer> nearbyNode5(int serverId){//获取更多的附近的节点
		...
		for(Map.Entry<Integer, Double> entry:mapTemp.entrySet()){
		...
		for(int i=0;i<nearBy.size();i++){
			double[] ne = Graph.nearbyNode0(nearBy.get(i));
			if(ne!=null){
				int key = (int) ne[0];
				double value = ne[1];
	...
	public static void updateServerLocation(List<Integer> serverLocation,List<Integer> sortedKey,Map<Integer,Integer> discard,List<Integer> unComplement){
		...
		for(;i<sortedKey.size()-discardNum;i++){
			int key = sortedKey.get(i);
			if(discard.containsKey(key) && discard.get(key)>1){//这里以前是1
				continue;
			}
			if(!serverLocation.contains(key)){
				serverLocation.add(key);		
		...
		for(int m=0;m<temp.size()-2;m++){//这里以前是2
			...
			if(discard.containsKey(in) && discard.get(in)>0){//这里之前是0
				continue;//出现过3次都被抛弃了，那就不要他了
			}
			if(!serverLocation.contains(in)){
				serverLocation.add(in);		
	...
	public static void loop21(List<Integer> serverLocation,long start,int s,int t){
		...
		while(times2>=0 && g4<totalLoop4){
			...
			if(nearByServer==null){
				...
				nearByServer = Graph.nearbyNode5(serverId);//这里以前是nearbyNode3
			}
			
			if(overall<nearByServer.size() && overall<8){			
	 ...
	 public static void initializeGraph(String[] data){
			...
			if(Graph.vertexNum>=500){
				...
				while(data[i]!=null && data[i].length()>0){
					...
					double everybandPrice = left_band/price;
					//维护一个邻接表
					if(Graph.map3.containsKey(id1)){
						double band = Graph.map3.get(id1);
					...
					if(Graph.map3.containsKey(id2)){
						double band = Graph.map3.get(id2);
			...
			if(Graph.vertexNum<500){
				...
				Map<Integer,Integer> aroundNum = new HashMap<Integer,Integer>();
				List<Integer> end = new ArrayList<Integer>();
				for(int k=0;k<Graph.consume.size();k++){		
					end.add(Graph.consume.get(k).linkedVertexId);	
				}
				
				int allWeight = 0;
				int allNum = 0;
				for(Map.Entry<Integer, Double> entry:Graph.map3.entrySet()){
					int key = entry.getKey();
					double value = entry.getValue();
					allWeight +=value;
					
					List<List<Integer>> result = Graph.searchRouteDFS(key,end);
					
					aroundNum.put(key, result.size());//将周围的消费节点个数也考虑进去
				...
				for(Map.Entry<Integer, Integer> entry:aroundNum.entrySet()){
					int key1  = entry.getKey();
					int value1 = entry.getValue();
					double norm_value1 = value1*1.0/allNum;
					
					double value2 = Graph.map3.get(key1);
					double norm_value2 = value2*1.0/allWeight;
					
					double weight = (norm_value1*0.5+norm_value2*0.5);
					Graph.map3.put(key1, weight);
				}
...
/*			//初始化部分修改
	...
	public static List<Integer> initializeServerLocation(Map<Integer,Integer> consumeMap){	
	  ...
	  public static List<List<Integer>> searchRouteDFS(int start,List<Integer> end){
			...
			lst.add(start);
			dfs2(result,start,end,lst,vertexlist);
		 ...
		 * @param graph
		 */
		private static void  dfs2(List<List<Integer>> result,int start,List<Integer> end,List<Integer> lst,Vertex[] vertexlist){
	...
	/*		if(lst.size() >=7){
			...
			*/
			...
			List<Integer> temp = Graph.mapVertex.get(start);
			...
			for(int i=0;i<temp.size();i++){
					...
					if(vertexlist[temp.get(i)].wasVisited == false){
						...
						dfs2(result,temp.get(i),end,lst,vertexlist);
