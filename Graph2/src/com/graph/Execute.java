...
/*	private static void  bfs(Map<Integer,List<Integer>> map,List<List<Integer>> result,int start,List<Integer> end,List<Integer> lst,Vertex[] vertexlist){
	...
	*/
	 ...
	 public static int calculate(AdjMatrixGraph graph,int[][] adjmatrixCopy,Map<Integer,Integer> serverRealProvide,List<Integer> serverLocation,Map<Integer,Integer> unsatisfied,Map<Integer,Integer> unsatisfiedInfo,int p){
	  ...
	  * @param maxProvide
	  */	 
	 public static void recursive(AdjMatrixGraph graph,List<Map.Entry<Integer, List<Integer>>> infoIds,List<Integer> serverLocation){
		    ...
		    while(p <= AdjMatrixGraph.consume.size()){
					...
					for(int j=0;j<10;j++){
						...
						int actualProvide = calculate(graph,adjmatrixCopy,serverRealProvide,serverLocation,unsatisfied,unsatisfiedInfo,p);				
						
						//将实际供应量中最小的那个找出来，准备剔除掉,这个后面可以再考虑一下
						int min_provide_id = removeServerIdx(serverRealProvide,serverLocation,actualProvide);
						...
						serverLocation.add(selectServerId);				
					...
					//为第二层迭代准备数据
				 	...
				 	for(int g=0;g<sortedUnsatisfied.size();g++){
				 		set.add(sortedUnsatisfied.get(g).getValue());
				 	}
				 	
				 	
					//第二层迭代逻辑：变更服务器的位置，这里层迭代p次，可以试情况而定
					for(int j=1;j<=p && j<set.size();j++){
						
						System.out.println("=============================================");
						System.out.println("=============================================");
						System.out.println("=============================================");
						System.out.println("第二层迭代逻辑:  "+" 选用"+p+"个服务器--------"+"第 "+ j + "次变更服务器的位置");
						System.out.println("the need of the Server number:   "+ p);
						System.out.println("the location of server:  "+serverLocation.toString());
						...
						int c=0;
						while(c<sortedUnsatisfied.size() && serverLocation.size()<j){
							int key = sortedUnsatisfied.get(c).getKey();
							int value = unsatisfiedInfo.get(key);
							if(!serverLocation.contains(value)){
								serverLocation.add(value);
							}
							c++;
						...
						int d=0,complement = 0;
						while(serverLocation.size()<p){
							complement = infoIds.get(d).getKey();
							if(!serverLocation.contains(complement)){
								serverLocation.add(complement);
							}
							d++;
						...
						Execute.totalMoney = 0;//核心功能的函数，用于寻找路径和计算费用，注意：下面的函数会更新Execute.totalMoney
						//使用启发式的方式，更新服务器的位置
						int actualProvide = calculate(graph,adjmatrixCopy,serverRealProvide,serverLocation,p);				
...
/*						if(serverChangeIdx==infoIds.size()){
						...
						serverLocation.add(selectServerId);	*/
						if(actualProvide>=AdjMatrixGraph.totalBand){
							System.out.println("在第二层逻辑满足条件，跳出！！！！！！！");
							break;
						}						
...
/*				System.out.println("找到的路径个数如下：=========" + result.size());
	 ...
	 public static void main(String[] args) throws FileNotFoundException {  
		...
		List<Integer> serverLocation = Execute.initializeServerLocation(graph,infoIds,1);
	 	Execute.recursive(graph,infoIds,serverLocation);
